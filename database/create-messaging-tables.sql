-- Create messaging tables from scratch for Alyne App
...
-- Allow database functions to work (SECURITY DEFINER functions)
CREATE POLICY "Allow function operations" ON conversations
    FOR ALL USING (true)
    WITH CHECK (true);

CREATE POLICY "Allow function operations" ON conversation_participants
    FOR ALL USING (true)
    WITH CHECK (true);
...
-- Go to Database > Realtime and enable for:
-- - conversations
